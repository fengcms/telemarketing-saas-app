...
1. [文档注释规范](#1-文档注释规范)
2. [文件结构规范](#2-文件结构规范)
   - 2.3 [单文件行数红线（560 行）](#23-单文件行数红线560-行)
3. [命名规范](#3-命名规范)
...
> 经 2026-07-24 第四轮代码审阅，组长裁定为**硬性底线**。

为控制单文件认知负荷与代码审阅成本，设定**硬性行数上限 560 行**（按 `wc -l` 统计文件总行数，含注释与空行）。

| 场景 | 规则 |
|------|------|
| 新建文件 | 预计超过 560 行时，必须先拆分（抽组件 / 抽方法 / 抽独立区块）再提交 |
| 既有文件 | 单次改动后若超过 560 行，须在本提交内同步拆分到阈值以下 |
| 例外 | **无例外**。列表页 / 详情页 / 表单等聚合型单屏同样遵守，靠抽子 Widget 与 `_build*` 方法达标 |

**拆分手法（参考）**：
- `build()` 只做编排，区块拆成 `_buildHeader()` / `_buildInfoCard()` / `_buildActionBar()` 等独立方法；
- 重复 UI 抽成独立 `StatelessWidget`（如 `ScheduleCard`、`TagChipRow`）；
- 长表单抽 `ScheduleFormContent` 之类的公共内容组件，由页面与抽屉共用；
- 同构逻辑抽工具 / helper（如 4 个操作 handler 抽 `_runAction(title, future)`）。

> **硬规则**：560 行红线是**不可商量的底线**，不是"建议"。代码审阅会把超限文件记为开放项并下调评级。若确因业务聚合度过高无法降到 560 以下，**必须先全体讨论修订阈值并写入本节**，而不是"规则存在但不守"。
...
> 经 2026-07-24 第四轮代码审阅，组长裁定为**硬性规则**。

凡是会导致**发布阻断**（release 装不上、起不来、核心流程走不通，如缺失必要权限、启动崩溃）的修复，必须**单独成一个提交**，不得与功能、样式、文档、工具改动混在同一提交。

```
# ❌ 厨房水槽：权限修复夹了 5 项不相关改动
fix(android): 在 main 清单声明 INTERNET 权限
    # 实际还改了 alice_manager / api_client / app.dart / 踩坑文档

# ✅ 拆开：阻塞级修复独占
fix(android): 在 main 清单显式声明 INTERNET 权限，修复 release 无法联网
docs: 补充 release/main 清单权限差异踩坑 §9      # 文档可单独立项，但别和功能修复捆死
```

理由：独占提交才能精准 `git cherry-pick` / 回滚 / `bisect`，避免为了带一个权限修复而被迫带上无关改动。

#### commit message 措辞如实（禁绝对词夸大）

> 经 2026-07-24 第四轮代码审阅，组长裁定为**硬性规则**。

subject 与 body 必须严格对应**实际改动范围**，禁止写「全部 / 统一 / 重构 / 重构全部」等绝对词，除非改动**真的**覆盖了该范围 100%。

```
# ❌ 夸大：实际只拆了 4 个文件，却写"全部巨型文件"
style: 全部巨型文件降至 560 行以下

# ❌ 夸大：实际仍有 12 个相对引用残留
refactor: 相对引用统一改为 package: 绝对引用

# ✅ 如实：写明实际范围
style: 4 个巨型文件降至 560 行以下（home/leads_list/follow_up/force_change）
refactor: leads_list 等 9 处相对引用改为 package: 绝对引用（剩余 12 处待补）
```

> 准确性既是诚信也是安全：`bisect` / `revert` 依赖 message 与 diff 一致；夸大的 message 会误导回溯。
